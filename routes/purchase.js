...
Router.get('/', (req, res) => {
  ...
  Model.Purchase.findAll(objFilterPurchase).then(function (purchase) {
...
Router.post('/add', (req, res) => {
  ...
  if (req.body.hotel_startdate != '' && req.body.hotel_enddate != '') {
    total_day =
      library.dayDiff(req.body.hotel_startdate, req.body.hotel_enddate) + 1;
  } else if (
    req.body.flightdeparture_startdate != '' &&
    req.body.flightdeparture_enddate != ''
  ) {
  ...
  Model.Cashflow.findAll({
    ...
    const dataCashflow = cashflow[0];
    const lastSaldo = dataCashflow ? dataCashflow.saldo : 0;
    ...
    if (lastSaldo > total_payment) {
      let objPurchase = {
        ...
        hotel_startdate:
          library.getMomentDate(req.body.hotel_startdate) || null,
        ...
        flightdeparture_startdate:
          library.getMomentDateTime(req.body.flightdeparture_startdate) || null,
        flightdeparture_enddate:
          library.getMomentDateTime(req.body.flightdeparture_enddate) || null,
        ...
        flightreturn_startdate:
          library.getMomentDateTime(req.body.flightreturn_startdate) || null,
        flightreturn_enddate:
          library.getMomentDateTime(req.body.flightreturn_enddate) || null,
              ...
              Model.Cashflow.create(objCashflow)
                .then(function () {
                  Model.User.findAll({
                    ...
                    let arrProfit = [];
                    for (let i = 0; i < investor.length; i++) {
                      let promiseProfit = new Promise(function (
                        resolve,
                        reject
                      ) {
                        let objProfit = {
      ...
      objAlert = message.error(
        'The remaining balance insufficient for payment !!'
      );
      res.redirect('/purchase/add/');
    }
  });
...
Router.post('/edit/:id', (req, res) => {
  ...
  if (req.body.hotel_startdate != '' && req.body.hotel_enddate != '') {
    ...
    req.body.flightreturn_startdate != '' &&
    req.body.flightreturn_enddate != ''
  ) {
    total_day =
      library.dayDiff(req.body.hotel_startdate, req.body.hotel_enddate) + 1;
  }
        ...
        if (lastSaldo > total_payment) {
          Model.Purchase.findOne({
            ...
            let objPurchase = {
              ...
              flightdeparture_airlinebook: req.body.flightdeparture_airlinebook,
              flightdeparture_airlinename: req.body.flightdeparture_airlinename,
              flightdeparture_startdate: library.getMomentDateTime(
                req.body.flightdeparture_startdate
              ),
              flightdeparture_enddate: library.getMomentDateTime(
                req.body.flightdeparture_enddate
              ),
              flightdeparture_airportfrom: req.body.flightdeparture_airportfrom,
              flightdeparture_airportto: req.body.flightdeparture_airportto,
              ...
              flightreturn_airlinebook: req.body.flightreturn_airlinebook,
              flightreturn_airlinename: req.body.flightreturn_airlinename,
              flightreturn_startdate: library.getMomentDateTime(
                req.body.flightreturn_startdate
              ),
              flightreturn_enddate: library.getMomentDateTime(
                req.body.flightreturn_enddate
              ),
              flightreturn_airportfrom: req.body.flightreturn_airportfrom,
              flightreturn_airportto: req.body.flightreturn_airportto,
  ...
  Model.Purchase.findOne({
    ...
    Model.Sale.findOne({
      ...
      if (!sale || purchase.status < 3) {
        ...
        objAlert = message.error(
          'Transaction rejected because has been used in another transaction !!'
        );
        res.redirect('/purchase');
      }
    });
